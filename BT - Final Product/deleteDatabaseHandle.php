<?php
	...
	if($deleteRight < $_SESSION['right']){
		...
		//Thành lập query
		if($requestData['extra'] == true){
			$query = 'DELETE FROM `'.$selectedTable.'`';
			$removeAllFlag = true;
		} else {
			$query = 'DELETE FROM `'.$selectedTable.'` WHERE ';
			$addAND = false;
		...
		if(mysqli_query($connector, $query)){
			...
			if($removeAllFlag == true){
				$returnArr['message'] = 'Đã xóa tổng cộng '.mysqli_affected_rows($connector).' hàng';
			} else {
				$returnArr['message'] = 'Hoàn tất xóa dữ liệu';
			}
			...
			mysqli_close($connector);
			$returnArr['message'] = 'Không thể xóa dữ liệu';
